...
import com.alex.diytomcat.http.StandardSession;
...
public class SessionManager {
    ...
    static {
        startSessionOutdatedCheckThread();
    }
    ...
    private static void startSessionOutdatedCheckThread() {
        new Thread() {
            public void run() {
                while (true) {
                    checkOutDateSession();
    ...
    private static int getTimeout() {
        ...
        try {
            Document d = Jsoup.parse(Constants.webXmlFile, "utf-8");
